 ...
 *
 * - Maps HTTP / contract error codes to user-friendly messages
...
export function friendlyMessage(error: unknown): string {
  ...
  if (e.code !== undefined) return MESSAGES[e.code] ?? e.message ?? MESSAGES.DEFAULT;
  if (e.status !== undefined) return MESSAGES[e.status] ?? MESSAGES.DEFAULT;
